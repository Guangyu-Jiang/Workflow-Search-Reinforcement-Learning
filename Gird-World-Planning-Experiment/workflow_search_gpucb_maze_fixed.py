"""
Workflow Search with GP-UCB on ObstacleMazeEnv using fixed training budgets.

Variant of the maze search script that trains each workflow for a fixed number
of PPO updates (no adherence-based early stopping) before evaluating GP-UCB.
"""
...
class AdherenceShapedMazeEnv(gym.Wrapper):
    """Wrapper for maze env with adherence-progress shaping and per-step penalty."""
    def __init__(self, env: gym.Env, workflow: List[int],
                 per_step_penalty: float,
                 adherence_bonus: float = 100.0,
                 adherence_penalty: float = 100.0,
...
def main():
    ...
    parser.add_argument('--updates-per-workflow', type=int, default=50,
                        help='Number of PPO updates to run for each workflow.')
    parser.add_argument('--num_envs', type=int, default=25)
    parser.add_argument('--max_steps', type=int, default=500)
    parser.add_argument('--n-steps', type=int, default=256,
                        help='Number of steps to collect per PPO update (default: 256, smaller = faster).')
    ...
    parser.add_argument('--adherence_penalty', type=float, default=100.0)
    parser.add_argument('--correct_entry_bonus', type=float, default=0)
    ...
    parser.add_argument('--allow_revisits', dest='allow_revisits', action='store_true', default=True)
    parser.add_argument('--continue_policy', action='store_true', default=True)
    parser.add_argument('--no-continue-policy', action='store_false', dest='continue_policy')
    parser.add_argument('--lr_decay', type=float, default=0.99)
    parser.add_argument('--min_lr', type=float, default=1e-8)
    parser.add_argument('--seed', type=int, default=0)
    parser.add_argument('--worker_seed_base', type=int, default=None)
    parser.add_argument('--exp_name', type=str, default='gpucb_maze_fixed')
    ...
    while True:
        ...
        # Optimize n_steps: collect fewer steps per update for faster training
        # n_steps should be much smaller than max_steps (episode length)
        # Typical: n_steps = 128-256 for faster collection
        n_steps_per_update = min(int(args.max_steps), int(args.n_steps))  # Use configurable n_steps
        
        sb3_model = SB3PPO(
            "MlpPolicy",
            vec_env,
            learning_rate=float(effective_lr),
            n_steps=n_steps_per_update,  # Reduced from max_steps
        ...
        # Use optimized n_steps instead of max_steps for faster collection
        steps_per_update = int(args.num_envs) * n_steps_per_update
        for update in range(int(args.updates_per_workflow)):
