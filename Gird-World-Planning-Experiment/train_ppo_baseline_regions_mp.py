"""
Multiprocessing PPO baseline on DiagonalRegionsEnv using env-only rewards.
...
def worker_episode(worker_id: int, policy_state_dict, max_steps: int, return_queue: mp.Queue):
    ...
    try:
        ...
        state_dim = 2 + 4 * 2 + 4  # agent(2) + region_centers(8) + visited_flags(4)
        policy = Policy(state_dim)
        policy.load_state_dict(policy_state_dict)
        policy.eval()

        env = DiagonalRegionsEnv(max_steps=max_steps)
        env.reset()
        ...
        while True:
            ...
            # Track first region entries
            pos = tuple(env.agent_pos)
            for reg_idx in range(env.num_regions):
                if env._in_region(pos, reg_idx) and reg_idx not in visited_sequence:
                    visited_sequence.append(reg_idx)
...
def main():
    ...
    parser.add_argument('--exp_name', type=str, default='ppo_baseline_regions_mp')
    ...
    # State dimension: agent(2) + region_centers(8) + visited_flags(4)
    state_dim = 2 + 4 * 2 + 4
